...
/**
 * Per active project member: the platform's weekly work-schedule template
 * (the same for every member — the schedule is global, not per-user) and
 * their next-14-days availability, with "occupied" counting only bookings
 * on *this* project (RD.md FR-8.8) — never another project's hours, task
 * or even its existence. Meant to be called only after the caller has
...
class CalculateProjectMemberCapacity
{
    ...
    public function handle(Project $project, Collection $members, CalculateUserAvailability $calculateUserAvailability): Collection
    {
        ...
        return $members->map(function (ProjectMember $member) use ($project, $schedule, $today, $twoWeeksOut, $calculateUserAvailability): array {
