...
export default function TimesheetIndex({
    ...
    const form = useHttp<{ week: string }, SubmittedResponse>({ week: weekStart });
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium" data-test="timesheet-week-range">
                            {weekStart} – {weekEnd}
                        </span>
                        <Button
                            variant="outline"
                    ...
                    <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {statusCounts.submitted > 0 ? (
                                <Badge variant="outline">{statusCounts.submitted} submitted</Badge>
                            ) : null}
                            ...
                            <Button
                                type="button"
                                size="sm"
                                disabled={!canSubmit || form.processing || !teamSlug}
                                onClick={submitWeek}
                    ...
                    <CardContent className="overflow-x-auto">
                        {hasRunningTimer ? (
                            <p className="text-muted-foreground mb-3 text-sm">
                                A running timer this week won't be included until it's stopped.
                            </p>
                        ) : null}

                        {rows.length > 0 ? (
                            <table className="w-full min-w-180 border-collapse text-sm" data-test="timesheet-grid">
                                <thead>
                                    <tr className="border-b text-left">
                                        <th className="py-2 pr-4 font-medium">Project · Activity</th>
                                        {days.map((day, index) => (
                                            <th key={day} className="w-20 py-2 text-right font-medium">
                                                {DAY_LABELS[index]}
                                                <br />
                                        ...
                                        <th className="w-20 py-2 text-right font-medium">Total</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {rows.map((row) => (
                                        ...
                                        >
                                            <td className="py-2 pr-4">
                                                ...
                                                <span className="capitalize">{row.activity_type.replace('_', ' ')}</span>
                                            </td>
                                            {days.map((day) => (
                                                <td key={day} className="text-right tabular-nums">
                                                    {row.days[day] > 0 ? row.days[day] : '–'}
                                                </td>
                                            ))}
                                ...
                                <tfoot>
                                    <tr className="font-medium">
                                        ...
                                        {days.map((day) => (
                                            <td key={day} className="pt-2 text-right tabular-nums">
                                                {dayTotals[day] > 0 ? dayTotals[day] : '–'}
                                            </td>
                                        ))}
                                        <td className="pt-2 text-right tabular-nums" data-test="timesheet-week-total">
                                            {weekTotal}
                                        </td>
                                    </tr>
                                </tfoot>
...
TimesheetIndex.layout = (props: { currentTeam?: { slug: string } | null }) => ({
    breadcrumbs: [
        {
            title: 'Timesheet',
            href: props.currentTeam ? timesheetIndex(props.currentTeam.slug) : '/',
        },
    ],
});
